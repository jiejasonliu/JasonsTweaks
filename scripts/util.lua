...
--[[
    limits the table to a certain size from m...n  

    @param table_data table to sub-table from
    @param m start of table (indices start at 1)
    @param n end of table
    @return a sub-table
]]

function util.get_table_size(table_data)
    local count = 0
...
function util.limit_table(table_data, m, n)
    return {table.unpack(table_data, m, n)}
end

function util.change_pos(position, delta_x, delta_y)
    return {x = position.x + delta_x, y = position.y + delta_y}
end

-- adds delta to each coord in the box
function util.expand_box(box, delta) 
    local left_top = box.left_top
    local right_bottom = box.right_bottom
    left_top.x = left_top.x - delta
    left_top.y = left_top.y - delta
    right_bottom.x = right_bottom.x + delta 
    right_bottom.y = right_bottom.y + delta
...
function util.shuffle(table)
    local t = util.deepcopy(table)
